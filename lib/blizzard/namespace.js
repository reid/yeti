...
BlizzardNamespace.prototype.bind = function () {
    ...
    panicDbg.set("blizzard-ns-bind-" + (new Date()), (new Error()).stack);
...
BlizzardNamespace.prototype.unbind = function () {
    if (!this.bound) {
        throw new Error("Not bound: " + this.id);
    }

    this.emit("end");

    panicDbg.set("blizzard-ns-unbind-" + this.id + "-" + (new Date()), (new Error()).stack);
