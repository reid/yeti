...
function WebDriverCollection(options) {
    ...
    this.url = options.hub.url || url.format({
...
WebDriverCollection.prototype._launch = function (desired, cb) {
    ...
    // We won't be sending Selenium commands very often,
    // after the tests have started, our next command will
    // be to close the browser.
    // Tell Sauce Labs that it should not timeout for a while.
    // https://saucelabs.com/docs/additional-config
    desired["idle-timeout"] = 5 * 60 * 60; // 5 hours
    desired["tags"] = ["yeti"];
