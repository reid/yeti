...
YUI.add("tempest", function (Y, name) {
    ...
    proto.emitBeat = function () {
        this.tower.emit("beat");
        this.lastSentBeats = this.get("beats");
    };
    ...
    proto.setupTowerHandlers = function () {
        ...
        self.tower.on("ready", function onReady() {
            ...
            (function keepalive() {
                ...
                if (self.get("captureOnly") || (self.lastSentBeats - self.get("beats")) > 0) {
                    self.emitBeat();
                    self.debug("Sent keepalive beat.");
                } else {
                    self.debug("Did not send beat; no activity.");
                }

                self.keepaliveTimeout = Y.later(3000, self, keepalive);
            }());
        });
