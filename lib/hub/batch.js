...
Batch.prototype.setupCleanupEvents = function () {
    ...
    self.session.on("end", function handleDeadClient() {
        if (process.env.TRAVIS) console.log("handling dead client",
            "runningAgents =", self.runningAgents);
        ...
        Object.keys(self.runningAgents).forEach(function unbind(id) {
            ...
            if (process.env.TRAVIS) console.log("removing agent after end", id);
            agent.dispatch([]);
        });
...
Batch.prototype.completeAgent = function (agent) {
    ...
    if (process.env.TRAVIS) console.log("agent complete", id, (new Error()).stack);
