...
/**
 * Serve a file from the Yeti Client on the given Hub.
 *
 * @method handleFileRequest
 * @param {Hub} server Yeti Hub.
 * @param {Number} agentId Agent ID.
 * @param {String} filename Filename requested.
 * @param {Function} [fileProcessor] File processor. If provided,
 *      this function will be called with the following arguments:
 * @param {Buffer} fileProcessor.buffer File contents, UTF-8 encoded.
 * @param {Function} fileProcessor.cb Callback. Must be called to complete the request
 *      with the following argument:
 * @param {Buffer} fileProcessor.cb.buffer Processed file contents, UTF-8 encoded.
 */
Batch.prototype.handleFileRequest = function (server, agentId, filename, fileProcessor) {
    var agent,
        batch = this;
    ...
    function serveBuffer(finalBuffer) {
        batch.testServer.serve(server, filename, finalBuffer);
    }
    ...
    this.getFile(filename, function (err, buffer) {
        ...
        if (fileProcessor) {
            fileProcessor(buffer, serveBuffer);
        } else {
            serveBuffer(buffer);
        }
    });
