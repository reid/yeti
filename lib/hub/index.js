...
/**
 * If listeners exist, the Hub will emit this event as files
 * are requested. The handler must process the file contents
 * and call the provided callback function for the Hub to
 * fufill the request.
 *
 * This function can be used to instrument JavaScript for code coverage.
 *
 * @event fileRequest
 * @param {String} filename Filename being served.
 * @param {Buffer} data File contents being served. UTF-8 encoded.
 * @param {Function} cb Callback. Must be invoked with the following argument:
 * @param {Function} cb.buffer Processed file contents, UTF-8 encoded.
 */
...
Hub.prototype._handleTestRequest = function (server, agentId, batchId, path) {
    var hub = this,
        ...
        fileRequestListeners = this.listeners("fileRequest");
    ...
    if (fileRequestListeners.length) {
        batch.handleFileRequest(server, agentId, file, function fileProcessor(buffer, cb) {
            hub.debug("fileRequest emit, filename =", file);
            hub.emit("fileRequest", file, buffer, cb);
        });
    } else {
