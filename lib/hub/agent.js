...
/**
 * Check if this Agent has not connected
 * since the given TTL.
 *
 * @method olderThan
 * @param {Number} ttl TTL in milliseconds.
 * @return {Boolean} True if the Agent is olderThan ttl, false otherwise.
 */
Agent.prototype.olderThan = function (ttl) {
    var age = Date.now() - this.seen;

    this.debug("olderThan check, age:", age, "ttl:", ttl);
    return age > ttl;
};
 ...
 */
Agent.prototype.getTTL = function () {
    var ttl = this.ttl;
...
Agent.prototype.expired = function () {
    return this.olderThan(this.getTTL());
};
 ...
 *
 * @method expired
 * @return {Boolean} True if the Agent is stale, false otherwise.
 */
Agent.prototype.stale = function () {
    return this.olderThan(Math.floor(this.getTTL() / 2));
};
...
AgentManager.prototype._startReap = function () {
    var interval = Math.floor(this.ttl / 2);
    if (this._reap) {
        this.debug("About to reschedule reap to internal =",
                interval + ", clearing old interval", this._reap);
        clearInterval(this._reap);
    }
    this._reap = setInterval(this.reap.bind(this), interval);
};
