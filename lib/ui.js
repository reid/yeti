...
function debug () {
    ...
    if (!verbose) return;
    var args = Array.prototype.slice.call(arguments);
    args.unshift(color.debug('  yeti debug - '));
    log.apply(this, args);
}
...
function info () {
    if (quiet) return;
    var args = Array.prototype.slice.call(arguments);
    args.unshift(color.info('  yeti info - '));
    log.apply(this, args);
}
...
function warn () {
    if (quiet) return;
    var args = Array.prototype.slice.call(arguments);
    args.unshift(color.warn('  yeti warn - '));
    log.apply(this, args);
}
...
function results (message) {
    ...
    //Not sure why this is a string yet, so I'm converting it here for now
    ...
    if (dots) {
        ...
        var re = ' (last test)';
        if (message.tests) {
            re = ' (' + message.tests + ' tests left to run)';
        }
        log(em(icon) + "  " + color.bold(result.name) + " on " + browser + re);
    }
