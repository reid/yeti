...
function submitBatch(client, options, cb) {
    ...
    batch.on("agentResult", function (agent, details) {
        ...
        if (details.coverage) {
            collector.add(details.coverage);
    ...
    batch.on("complete", function () {
        ...
        var duration = Number(new Date()) - timeStart,
            file = require("path").join(process.cwd(), "coverage.json"),
            total = batchDetails.passed + batchDetails.failed,
            durationString = "(" + duration + "ms)";

        require("fs").writeFileSync(file,
            JSON.stringify(collector.getFinalCoverage()), "utf8");
        error("Wrote Istanbul coverage results to", file);
        error("Generate HTML reports by running `istanbul report`.");
...
function runBatch(options) {
    ...
    function createHub() {
        ...
        if (coverage) {
            error("Setting up Istanbul code coverage instrumenter for this Hub.");

            instrumenter = new Instrumenter({
                coverageVariable: "_yuitest_coverage"
            });

            hub.on("fileRequest", function (filename, data, cb) {
                if (!/\.js$/.test(filename)) {
                    return cb(data);
                }
                error("Instrumenting", filename);
                data = instrumenter.instrumentSync(data.toString("utf8"), filename);
                cb(new Buffer(data, "utf8"));
            });
        }
