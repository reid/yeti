...
function putsTravis() {
    ...
    if (TRAVIS) {
        ...
        args.unshift("[Yeti Travis Debug]");
        console.log.apply(console, args);
    }
}
...
if (isTTY) {
    putsTravis("stderr is a TTY, which does not make sense on Travis.");
    if (TRAVIS) {
        error("Warning: Overriding isTTY to false.");
        isTTY = false;

        error("Warning: Setting debugging environment variables.");
        process.env.YETI_HUB_DEBUG = 1;
        process.env.BLIZZARD_DEBUG = 1;
    }
} else {
    putsTravis("stderr is not a TTY, which is expected on Travis.");
}
