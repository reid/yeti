...
function serveExpress (port, path, cb) {
    ...
    socket.set('close timeout', '60');
    socket.set('polling duration', '60');
    socket.set('close timeout', '45');
    socket.set('heartbeat timeout', '30');
    socket.set('heartbeat interval', '45');
	...
	var ip = ((client.connection && client.connection.remoteAddress) ? ' from ' + client.connection.remoteAddress : '');
        console.log("Yeti loves " + client.id + ip);
        client.on("message", function (message) {
			...
			var c = client.listener.clients[id]; //Client is undefined
			if (!c) {
				console.log('Removing stale client session: ' + id);
				ret = !!c;
			}
                        return ret;
                    });
